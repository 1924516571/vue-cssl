      ...
      >
        <template v-for="item in columns">
          <template v-if="item.slot">
            <suc-table-column :type="item.type" :prop="item.key" :label="item.title" :width="item.width" :key="item.id">
          ...
          <template v-else>
            <suc-table-column :type="item.type" :prop="item.key" :label="item.title" :width="item.width" :key="item.id"></suc-table-column>
          </template>
        </template>
      </suc-table>
    </div>
...
export default class ModuleView extends Vue {
  ...
  @Prop() customHeight: any; //父组件传一个列表高度属性过来，子页面监听值的变化
