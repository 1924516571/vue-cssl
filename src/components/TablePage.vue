<template>
  ...
  <div class="table-view">
    <div>
      <suc-table :data="tableData" :height="tableHeight" stripe :size="size" :row-class-name="tableRowClassName" @row-click="rowClick" @sort-change="sortChange">
        <template v-for="item in columns">
          <template v-if="item.slot">
            <suc-table-column :type="item.type" :prop="item.key" :label="item.title" :width="item.width" :key="item.id" :sortable="item.sortable" :fixed="item.fixed">
              <template slot="header">
                <slot :name="item.heading"></slot>
              </template>
              <template slot-scope="scope">
                <slot :name="item.slot" :row="scope.row" :index="scope.$index" v-bind:data="{ row: scope.row, index: scope.$index }"></slot>
              </template>
            </suc-table-column>
          </template>
