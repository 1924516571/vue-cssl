...
const utils = {
  ...
  getTodayDate: () => {
    ...
    if (nowDate.getTime() < nowEightTime) {
      return [new Date(new Date(new Date().getTime() - 24 * 60 * 60 * 1000).setHours(8, 0, 0, 0)), nowDate.getTime()];
    } else {
      return [new Date(new Date().getTime()).setHours(8, 0, 0, 0), nowDate.getTime()];
      // new  Date(); 标准时间
    //   nowDate.getTime()  这个打印出来是时间戳
...
export default {
  install: (Vue: any, options: any) => {
    ...
    Vue.component("v-chart", ECharts); //自定义组件 vue中有install  方法
