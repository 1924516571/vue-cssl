  ...
  <div class="info">
    <div class="info-view bg-shadow">
      <div class="top">
        ...
        <div class="top-item">
          <el-upload type="success" size="small" :headers="uploadHeaders" :on-success="PicSuccess" action="api/wjServer/dataCenter/tFcZdhd/import" accept=".xls,.xlsx" :show-file-list="false">
      ...
      <div class="content">
        <z-table showTotal :tableData="tableData" v-loading="loading" :pageParams="pageParams" @get-page="getPage" @get-size="getSize">
          <template v-slot:column>
            ...
            <el-table-column prop="name" label="河道名称"></el-table-column>
            <el-table-column prop="length" label="河道长度(km)"></el-table-column>
            <el-table-column prop="hezhang" label="河长"></el-table-column>
            <el-table-column prop="service" label="河长职务"></el-table-column>
            <el-table-column prop="hzsub" label="分河长"></el-table-column>
            <el-table-column prop="duty" label="分河长职务" width="200"></el-table-column>
    ...
    <imp-river-modal :title="modalTitle" :model="model" :loading="loading" :formParams="formParams" @on-submit="oncreate" @toggle="toggle"></imp-river-modal>
...
import { Create, DeleteModal, LinkYj, LinkDefault, HeDaoModal, LinkWithModal, HdDetailModal, ImpRiverModal } from "../model";
...
export default class Enterprise extends Vue {
  ...
  // 导出
  exportExcel() {
    if (this.stationParams.town == undefined) {
      this.stationParams.town = "";
    }
    let excelUrl = url + "/api/wjServer/dataCenter/tFcZdhd/export?town=" + this.stationParams.town + "&name=" + this.stationParams.name;
    window.open(excelUrl);
  }

  // 模板下载
  downLoad() {
    location.href = "api/wjServer/重点河道基本情况汇总表模板.xls";
  }
  ...
  async save() {
    ...
    let url = "wjServer/dataCenter/tFcZdhd/delInfo";
  ...
  async setCreate() {
    ...
    let a = {
      rvcd: this.formParams.code,
    };
    let checkData = await publicApi.getWithParam(checkUrl, a);
    if (checkData.data) {
      let url = "wjServer/dataCenter/tFcZdhd/addInfo";
  ...
  async setEdit() {
    this.loading = true;
    let url = "wjServer/dataCenter/tFcZdhd/editInfo";
  ...
  async editInfo(params: any) {
    let url = "wjServer/dataCenter/tFcZdhd/getInfoById";
    let detailParams = {
      id: params,
    };
  ...
  async onpage() {
    ...
    let url = "wjServer/dataCenter/tFcZdhd/infoList";
