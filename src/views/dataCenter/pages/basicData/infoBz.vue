<template>
  ...
  <div class="info">
    <div class="info-view bg-shadow">
      <div class="top">
        <div class="top-item">
          ...
          <div class="top-item-sub">
            <suc-select v-model="stationParams.zhantype" :options="shuizhatypeData" :config="{ placeholder: '请选择泵站类型', clearable: true }" style="width:160px"></suc-select>
          </div>
          <div class="top-item-sub">
        ...
        <div class="top-item">
          <el-upload type="success" size="small" :headers="uploadHeaders" :on-success="PicSuccess" action="api/wjServer/dataCenter/tFcBengZhan/import" accept=".xls,.xlsx" :show-file-list="false">
      ...
      <div class="content">
        <z-table showTotal :tableData="tableData" v-loading="loading" :pageParams="pageParams" @get-page="getPage" @get-size="getSize">
          <template v-slot:column>
            ...
            <el-table-column prop="bengmc" label="泵站名称"></el-table-column>
            <el-table-column prop="lwmc" label="所在联圩"></el-table-column>
            <el-table-column prop="river" label="所在河道"></el-table-column>
            <el-table-column prop="zhantype" label="泵站类型"></el-table-column>
            <el-table-column prop="num" label="机组台数"></el-table-column>
            <el-table-column prop="capacity" label="装机容量（kw）"></el-table-column>
            <el-table-column prop="desnum" label="设计流量(m³/s)"></el-table-column>


            <el-table-column prop="insize" label="进水池尺寸(m²)"></el-table-column>
            <el-table-column prop="outsize" label="出水池尺寸(m²)"></el-table-column>
            <el-table-column prop="gztime" label="新建时间改建"></el-table-column>
    ...
    <bz-modal :title="modalTitle" :model="model" :loading="loading" :formParams="formParams" @on-submit="oncreate" @toggle="toggle"></bz-modal>
...
import { Create, DeleteModal, LinkYj, LinkDefault, HeDaoModal, LinkWithModal, HdDetailModal, ImpRiverModal, NqModal, SzModal,BzModal } from "../model";
...
export default class Enterprise extends Vue {
  ...
  // 导出
  exportExcel() {
    if (this.stationParams.town == undefined || this.stationParams.village == undefined || this.stationParams.zhantype == undefined) {
      this.stationParams.town = "";
      this.stationParams.village = "";
      this.stationParams.zhantype = "";
    }
    let excelUrl =
      url +
      "/api/wjServer/dataCenter/tFcBengZhan/export?town=" +
      this.stationParams.town +
      "&village" +
      this.stationParams.village +
      "&zhantype=" +
      this.stationParams.zhantype +
      "&bengmc=" +
      this.stationParams.bengmc;
    window.open(excelUrl);
  }

  // 模板下载
  downLoad() {
    location.href = "api/wjServer/泵站基本情况汇总表模板.xls";
  }
  ...
  async save() {
    ...
    let url = "wjServer/dataCenter/tFcBengZhan/delInfo";
  ...
  async setCreate() {
    this.loading = true;
    let url = "wjServer/dataCenter/tFcBengZhan/addInfo";
  ...
  async setEdit() {
    this.loading = true;
    let url = "wjServer/dataCenter/tFcBengZhan/editInfo";
  ...
  async onpage() {
    ...
    let url = "wjServer/dataCenter/tFcBengZhan/infoList";
