<template>
  ...
  <div class="info">
    <div class="info-view bg-shadow">
      <div class="top">
        <div class="top-item">
          ...
          <!-- 河湖名称 -->
          <div class="top-item-sub">
            <search-component placeholderTxt="桥名称" clearable="false" @input="onSearch"> </search-component>
          </div>
        </div>
        <div class="top-item">
          <el-upload type="success" size="small" :headers="uploadHeaders" :on-success="PicSuccess" action="api/wjServer/dataCenter/tFcBridge/import" accept=".xls,.xlsx" :show-file-list="false">
      ...
      <div class="content">
        <z-table showTotal :tableData="tableData" v-loading="loading" :pageParams="pageParams" @get-page="getPage" @get-size="getSize">
          <template v-slot:column>
            ...
            <el-table-column prop="bname" label="桥名称"></el-table-column>
            <el-table-column prop="lwmc" label="所在联圩"></el-table-column>
            <el-table-column prop="river" label="所在河道"></el-table-column>
            <el-table-column prop="bnature" label="桥梁性质"></el-table-column>
            <el-table-column prop="jztime" label="建造或改建时间"></el-table-column>
            <el-table-column prop="byupe" label="桥梁类型"></el-table-column>
            <el-table-column prop="buser" label="桥梁用途"></el-table-column>
            <el-table-column prop="length" label="净长"></el-table-column>
            <el-table-column prop="wid" label="净宽"></el-table-column>
            <el-table-column prop="thgc" label="通航高程(梁底高程)"></el-table-column>
    ...
    <nq-modal :title="modalTitle" :model="model" :loading="loading" :formParams="formParams" @on-submit="oncreate" @toggle="toggle"></nq-modal>
...
import { Create, DeleteModal, LinkYj, LinkDefault, HeDaoModal, LinkWithModal, HdDetailModal, ImpRiverModal,NqModal } from "../model";
...
export default class Enterprise extends Vue {
  ...
  // 导出
  exportExcel() {
    if (this.stationParams.town == undefined || this.stationParams.village == undefined) {
      this.stationParams.town = "";
      this.stationParams.village = "";
    }
    let excelUrl = url + "/api/wjServer/dataCenter/tFcBridge/export?town=" + this.stationParams.town + "&village" + this.stationParams.village + "&bname=" + this.stationParams.bname;
     window.open(excelUrl);   
  }

  // 模板下载
  downLoad() {
    location.href = "api/wjServer/农桥基本情况汇总表模板.xls";
  }
  ...
  async save() {
    ...
    let url = "wjServer/dataCenter/tFcBridge/delInfo";
  ...
  async setCreate() {
    ...
    let a = {
      rvcd: this.formParams.bh,
    };
    let checkData = await publicApi.getWithParam(checkUrl, a);
    if (checkData.data) {
      let url = "wjServer/dataCenter/tFcBridge/addInfo";
      ...
      this.model = false;
      this.$SucMessage.error("农田编号已存在,重新输入");
    }
    this.onpage();
  }
  // 更新
  ...
  async editInfo(params: any) {
    let url = "wjServer/dataCenter/tFcBridge/getInfoById";
    let detailParams = {
      id: params,
    };
  ...
  async onpage() {
    ...
    let url = "wjServer/dataCenter/tFcBridge/infoList";
