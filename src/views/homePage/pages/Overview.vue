<template>
  <div class="module-view">
        ...
        <template v-slot:markerlayer>
          <div>
            <div v-if="MapInfo.show">
              <ol-overlay :position="MapInfo.coords" :offset="MapInfo.offset" category="clickLabel" ref="test">
    ...
    <div :class="flag ? 'tc' : 'tc1'">
      <overview-panel :sendType="sendType" ref="child" @get-wq="getWq" @getSx="getSx" @childClick="getClickType"></overview-panel>
    </div>
...
import { OverviewPanel, YlzModal, SwModal, ZbzModal, WzckModal } from "./overviewPage";
import { coordinateTransform } from "@suc/monch";
import { RightBar } from "./MonitorPage";
import { getBzIcon, getZmIcon, getBjIcon, getYlzIcon, getSwzIcon } from "@/constants/mapIcon";
...
export default class OverView extends Vue {
  ...
  getSx(sx: any) {
    this.olMap.wjLayer[1].visible = sx; //河道面
    this.olMap.wjLayer[2].visible = sx; //区域河道
    this.olMap.wjLayer[3].visible = sx; //流域河道
    this.olMap.wjLayer[4].visible = sx; //市级河道
  }
  ...
  onTc() {
    ...
    if (this.arrowLeft) {
      ...
      this.jcflag = false;
      this.flag = true;
      (this.$refs.child as any).getSwPoint();
    }
  }
  // 监测点击
  onJc() {
  ...
  getGisData(gisdata: any, icon: any) {
    ...
    let arr = gisdata.map((item: any) => {
      ...
      if (icon == "泵站") {
        ...
        // src = getBzIcon(item.rangeKey,"在线");
        ...
        if (item.overWarn) {
          src = require("@/assets/cs/iconswz/dw-sw3.png");
        } else {
  ...
  async getSwzLine() {
    ...
    this.realWater = data.data.z; //实时水位

    // this.options.series[0].markLine.data[0].yAxis = data.data.warnZ || 0;
    this.warnZ = data.data.warnZ;

    if (this.hisInfo.length > 0) {
      const resversArr = this.hisInfo.reverse();
      resversArr.forEach((e: any) => {
