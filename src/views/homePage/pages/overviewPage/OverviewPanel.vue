      ...
      <template v-if="flag">
        <div class="img-view scroll">
          ...
          <div class="check-item" v-if="type == 0">
            ...
            <!-- 河道 -->
            <!-- <div class="yq_content">
              <el-checkbox class="boxStyle" :indeterminate="isIndeterminate" v-model="allCheckHl" @change="getAllhl">河道</el-checkbox>
              <el-checkbox-group v-model="checkboxGroupValue1" @change="getHlCheck">
                <div class="main-checkbox">
                  <el-checkbox v-for="city in cities" :label="city.label" :key="city.label"><span :class="city.icon"></span>{{ city.value }}</el-checkbox>
                </div>
              </el-checkbox-group>
            </div> -->
            <!-- 易涝点 -->
...
export default class OverviewPanel extends Vue {
  ...
  // 点击图层从新刷新tabs栏，传了一个true or  false
  ...
  mounted() {
    ...
    //   // setTimeout(this.getSwPoint, 0);
    //   this.getSwPoint();
    // }, 1000 * 60 * 5);
    // this.Yltimer = setInterval(() => {
    //   // setTimeout(this.getRainPoint, 0);//
    //   this.getRainPoint();
    // }, 1000 * 60 * 5);
  }
  beforeDestroy() {
